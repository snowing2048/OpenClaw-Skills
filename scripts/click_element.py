...
# Disable pyautogui fail-safe for automated operations
...
def find_and_click(desktop, element_name, window_title=None, control_type=None, exact=False):
    ...
    target = candidates[0]
    try:
        # Use click_input() which works for more control types
        target['control'].click_input()
        return True, f"Clicked [{target['type']}] '{target['name']}' in {target['window']} @ {target['center']}"
    except Exception as e:
        # Fallback: try .invoke() if click_input fails
        try:
            target['control'].invoke()
            return True, f"Clicked via invoke [{target['type']}] '{target['name']}'"
        except Exception as e2:
            return False, f"Click failed: {e}, invoke failed: {e2}"
